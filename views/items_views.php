...
if (isset($_GET["delete"])) {
	...
	$delete="DELETE FROM products WHERE Item_id='$Item_id'";
	try {
	...
	<?php
		...
		<form action="" method="POST">
			<div class="w3-container " id="print_sel">
				...
				<?php echo $notice;?>
				<div class="w3-container w3-white" ><h1><strong>ALL ITEMS </strong></h1></div>
				<table class="w3-table-all w3-left" id="table">
					<tr class="w3-red ">
						<th class="w3-red">N<label style="text-decoration: underline;">&#111;</label></td>
						<th>ITEM TYPE</th>
						<th>NAMES</th>
						<th>E-MAIL</th>
						<th>TELEPHONE</th>
						...
						<th>ACTION</th>
								...
								<?php
										...
										$SEARCH=$connection->prepare("SELECT * FROM products limit $pageno,$rowperpage");
										$SEARCH->execute();
										
										 foreach ($SEARCH->fetchAll() as $row) {?>
										 			
										 			 <tr class='item'>
										 			 <td class='w3-red'><?php echo $row["Item_id"];?></td>
										 			 <td class=''><?php echo $row["Item_type"];?></td>
										 			<td class=''><?php echo $row["Owner_names"];?></td>
										 			<td class=''><?php echo $row["E_mail"];?></td>
										 			<td class=''><?php echo $row["Telephone"];?></td>
								                    <td class=''><?php echo $row["Item_cost"];?> Frw</td>
								                    <td class=''><img style='width:30px;' class='w3-round-xxlarge' src="<?php echo $row["Item_photo"];?>"></td>
								                    <td class=''><?php echo $row["Status"]?></td>
								                     <td>
													 	<?php echo "<a title='Click to Delete this Items. ' href='items_views.php?Item_id=".$row['Item_id']."&delete' class='w3-btn w3-round-xxlarge w3-red w3-pointer'><i class='fa fa-remove'></i></a>"; ?>
													 </td>
								                </tr>
								            <?php  }
						?>
