	...
	<?php
		...
		<form action="" method="POST">
			<div class="w3-container">
				...
				<?php echo $notice;?>
				<div class="w3-container w3-white"><h1><strong>SELLING STATUS </strong></h1></div>
					...
					<tr class="w3-red ">
						...
						<td>ACTION</td>
								...
								if ($_SESSION["User_type"]=="Customer") {
									...
									$sql = "SELECT COUNT(*) FROM products WHERE Cust_id='$buyer'";
									$SEARCH=$connection->prepare("SELECT * FROM products WHERE Cust_id='$buyer' limit $pageno,$rowperpage");
								} else {
									$buyer=$_SESSION["User_id"];
									$sql = "SELECT COUNT(*) FROM products WHERE User_id='$buyer'";
									$SEARCH=$connection->prepare("SELECT * FROM products WHERE User_id='$buyer' limit $pageno,$rowperpage");
								}
								...
								$i=0;
										...
										$SEARCH->execute();
										 foreach ($SEARCH->fetchAll() as $row) {
										 			$i++;
										 			 echo"<tr class='item'><td class='w3-red'>".$row["Item_id"].
										 			"<td class=''>".$row["Item_type"].
										 			"</td><td class=''>".$row["Owner_names"].
										 			"</td><td class=''>".$row["E_mail"].
								                    "</td><td class=''>".$row["Telephone"].
								                    "</td><td class=''>".$row["Item_cost"].
								                    " Frw</td><td class=''><img style='width:30px;' class='w3-round-xxlarge' src=".$row["Item_photo"].
								                    "></td><td class=''>".$row["Status"].
								                    "</td><td><a  title='Click to Edit this Item. ' href='item_edit.php?Item_id=".$row['Item_id']."&edit' class='w3-btn w3-margin-right w3-round-xxlarge w3-green w3-pointer '><i class='fa fa-edit'></i></a>".
								                    "<a title='Click to Delete this Item. ' href='item_edit.php?Item_id=".$row['Item_id']."&delete' class='w3-btn w3-round-xxlarge w3-red w3-pointer'><i class='fa fa-remove'></i></a></tr>";
								                }
						?>
