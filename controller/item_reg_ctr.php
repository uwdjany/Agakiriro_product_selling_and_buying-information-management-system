		...
		try {
			...
			if (empty($photo)) {
				$notice="<script>alert('the photo field is empty or rename the photo before uploading it then it will accept!!')</script>";
					...
					$insert="INSERT INTO `products`(`Item_type`, `Owner_names`, `E_mail`, `Telephone`, `Item_cost`, `Item_photo`, `User_id`,`Status`)
					 VALUES ('$item_type','$names','$Email','$Tel','$item_cost','$photo','$User_id','Appending')";
